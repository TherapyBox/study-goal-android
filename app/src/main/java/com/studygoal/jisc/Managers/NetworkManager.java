...
//                        if(activityHistory.module_id.length() != 0 && activityHistory.activity_type.length() != 0 && activityHistory.activity.length() != 0)
    ...
    private class getEventsAttended implements Callable<String>{
        private int skip;
        private int limit;

        getEventsAttended(int skip, int limit){
            this.skip = skip;
            this.limit = limit;
        }
        ...
        public String call(){
            try {
                ...
                if (responseCode != 200) {
                    if (responseCode == 204) {
                        Log.i("getEventsAttended", "No records found");
                    } else {
                        Log.e("getEventsAttended", "Code: " + responseCode);
                    }
                    return "Error";
                }
                ...
                SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(appContext);
                SharedPreferences.Editor editor = prefs.edit();
                editor.putString(appContext.getString(R.string.events_attended), sb.toString());
                editor.apply();
                Log.e("Events attended", sb.toString());
                return "Success";
            } catch (Exception e) {
                e.printStackTrace();
                return "Error";
